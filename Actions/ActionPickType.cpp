...
#include <iostream>
ActionPickType::ActionPickType(ApplicationManager* pApp) :Action(pApp) {
	no_shapes = 0;
	rightAns = 0;
	wrongAns = 0;
}
void ActionPickType::Execute() {
	...
	for (int i = 0; i < pManager->getFigCount(); i++) {
		...
		else if (Fig->getFigureName() == "HEXAGON") {
			shapes[2]++;	
		}
		else {
			shapes[3]++;	
...
void ActionPickType::score(int _score) {
	GUI* pGUI = pManager->GetGUI();
	string msg;
	if (_score == 1) {
		rightAns++;
